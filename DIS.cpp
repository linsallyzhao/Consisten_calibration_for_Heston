...
int main() {
    double allT[] = {0.0, 0.01917808, 0.03835616, 0.05753425, 0.08219178,
    ...
    double disInitial[] = {0.91840917, 0.50851988, 0.25907174, 0.85560232, 0.53713517,
    ...
    const VD VIXkarr = {0.07660333466800909, 0.07823036262505452, 0.08007866879151475,
    ...
    const VD VIXtarr = {0.01917808, 0.01917808, 0.01917808, 0.01917808, 0.01917808,
                    0.03835616, 0.03835616, 0.03835616, 0.03835616, 0.03835616,
                    0.05753425, 0.05753425, 0.05753425, 0.05753425, 0.05753425,
                    0.08219178, 0.08219178, 0.08219178, 0.08219178, 0.08219178,
                    0.16438356, 0.16438356, 0.16438356, 0.16438356, 0.16438356,
    ...
    const VD SPXtarr = {0.08219178, 0.16438356, 0.24657534, 0.32876712,
                        0.4109589 ,0.49315068, 0.73972603, 0.98630137,
                        0.08219178, 0.16438356, 0.24657534, 0.32876712,
                        0.4109589 ,0.49315068, 0.73972603, 0.98630137,
                        0.08219178, 0.16438356, 0.24657534, 0.32876712,
                        0.4109589 ,0.49315068, 0.73972603, 0.98630137,
                        0.08219178, 0.16438356, 0.24657534, 0.32876712,
...
//                     0.595238095238095, 0.714285714285714, 1.07142857142857,
//                     1.42857142857143};
//                     //These are maturities divided by 252. Not sure why not 365, but this is just a normalization anyway.
//
    ...
    //std::cout << "Parameters:" << "\t         kappa"<<"\t     vinf"<< "\t       v0"<< "\t      rho" << "\t     vov"<<std::endl;
    ...
    //std::cout << "Real optimum:" << "\t" << mp.k<<"\t"<< mp.vbar<< "\t"<< mp.v0<< "\t"<< mp.rho<< "\t"<< mp.sigma << std::endl;
...
void objFunc (double *p, double *x, int m, int n, void *data){
    ...
    VD SPXprices = SPXprice(molp, mktp->SPX_T, mktp->S, mktp->SPX_K, mktp->r, (int)mktp->SPX_T.size());
    VD VIXprices = VIXprice(molp, mktp->VIX_T, mktp->S, mktp->VIX_K, mktp->r, (int)mktp->VIX_K.size());
...
void JacFunc (double *p, double *jac, int m, int n, void *data){
    ...
    modelPara molp = {p[0], p[1], p[2], p[3], p[4]};
    VD SPXjac = gradientSPXprice(molp, mktp->S, mktp->r, (int)mktp->SPX_T.size(), mktp->SPX_T, mktp->SPX_K);
    VD VIXjac = gradientVIXprice(molp, mktp->r, (int)mktp->VIX_T.size(), mktp->VIX_T, mktp->VIX_K, mktp->tbar);
...
intVIXdata VIXintegrand(CD u, modelPara p, double tau, double K, double tbar){
    ...
    intVIXdata ret = {VIXint, rawInt, atauBar, tmp.G, tmp.F, inputU, iu, start_count, end_count};
