...
const double pi = 4.0*atan(1.0), lb = 0.0, ub = 200, mid = 0.5*(ub + lb), halfRange = 0.5*(ub - lb);
...
double SPXintegrand(double u, modelPara p, double tau, double K, double S, double r);
...
CD CFprice(CD u, modelPara p, double tau, double S, double r){
    ...
    CD CF = exp(iu*(log(S)+r*tau) - tmp*rho*tau*iu - A + 2*tmp*D/sigma);
