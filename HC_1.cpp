...
// an example of using this calibrator to find the presumed optimal parameter
// set.
...
// Bibtex Entry:
// @article{Cui2015,
// title = "Full and fast calibration of the {H}eston stochastic volatility
// model", author = "Yiran Cui and Sebastian del Ba\~{n}o Rollin and Guido
// Germano", journal = "arXiv:1511.08718", year = 2015}
...
// can ensure 1e-8 accuracy for the option price. that is, pvtrapz - pvgauss
// <=1e-8.
// static double x64[32] =
...
const double pi = 4.0 * atan(1.0), lb = 0.0, ub = 200, Q = 0.5 * (ub - lb),
             P = 0.5 * (ub + lb);

// market parameters: you may change the number of observations by modifying the
// size of T and K
struct mktpara {
    double S;
    double r;
...
tagMN HesIntMN(double u, double a, double b, double c, double rho, double v0,
    ...
    complex<double> h_M =
        pow(K, -imPQ_M) /
        imPQ_M;  // Lin: equation (9), exp(-iu)*K/(iu) instead of K^(-iu)/(iu)
    ...
    // m = i*u1 + pow(u1,2);
    complex<double> m_M1 =
        imPQ_M + one + pow(PQ_M - i, 2);  // m_M1 = (PQ_M-i)*i + pow(PQ_M-i, 2);
    complex<double> m_N1 =
        imPQ_N + one + pow(PQ_N - i, 2);  // m_N1 = (PQ_N-i)*i + pow(PQ_N-i, 2);
    complex<double> m_M2 =
        imPQ_M +
        pow(PQ_M - zero * i, 2);  // Lin: why -zero*i? to convert it into
                                  // complex? Print out type before and after
    ...
    // characteristic function: y1 = exp(i*x0*u1) * exp(-v0*A) * g *
    // exp(2*a*b/pow(c,2)*D)
    tmp = 2 * a * b / csqr;
    double halft = 0.5 * T;
    complex<double> D_M1 =
        log(d_M1) + (a - d_M1) * halft -
        log((d_M1 + kes_M1) * 0.5 + (d_M1 - kes_M1) * 0.5 * exp(-d_M1 * T));
    complex<double> D_M2 =
        log(d_M2) + (a - d_M2) * halft -
        log((d_M2 + kes_M2) * 0.5 + (d_M2 - kes_M2) * 0.5 * exp(-d_M2 * T));
    complex<double> D_N1 =
        log(d_N1) + (a - d_N1) * halft -
        log((d_N1 + kes_N1) * 0.5 + (d_N1 - kes_N1) * 0.5 * exp(-d_N1 * T));
    complex<double> D_N2 =
        log(d_N2) + (a - d_N2) * halft -
        log((d_N2 + kes_N2) * 0.5 + (d_N2 - kes_N2) * 0.5 * exp(-d_N2 * T));
...
void fHes(double *p, double *x, int m, int n, void *data) {
    ...
    struct mktpara *dptr;
    dptr = (struct mktpara *)data;  // Lin: don't understand what is going on
                                    // here, assuming data contains market paras
    double S = dptr->S;
    double r = dptr->r;
    ...
    NumGrids = (NumGrids + 1) >> 1;  // Lin: shift? why doing this?
    double *u = glaw.u;
    double *w = glaw.w;
...
tagMNJac HesIntJac(double u, double a, double b, double c, double rho,
    ...
    complex<double> h_M =
        pow(K, -imPQ_M) / imPQ_M;  // Lin: same quesions as above
    complex<double> h_N = pow(K, -imPQ_N) / imPQ_N;
    ...
    complex<double> m_M1 =
        imPQ_M + one + _msqr;  //    m_M1 = (PQ_M - i)*i + pow(PQ_M-i, 2);
    complex<double> m_N1 =
        imPQ_N + one + _nsqr;  //    m_N1 = (PQ_N - i)*i + pow(PQ_N-i, 2);
    ...
    complex<double> y4M1 =
        -v0 * pA_prho_M1 + tmp3 * ctmp / d_M1 + tmp1 * _imPQ_M;
    ...
    complex<double> y4N1 =
        -v0 * pA_prho_N1 + tmp3 * ctmp / d_N1 + tmp1 * _imPQ_N;
    ...
    complex<double> y4M2 =
        -v0 * pA_prho_M2 + tmp3 * ctmp / d_M2 + tmp1 * imPQ_M;
    ...
    complex<double> y4N2 =
        -v0 * pA_prho_N2 + tmp3 * ctmp / d_N2 + tmp1 * imPQ_N;
    ...
    complex<double> y5M1 = -v0 * pA_prho_M1 * ctmp + tmp2 * lnB_M1 +
                           a * tmp2 * pB_pa / B_M1 - tmp1 * _imPQ_M;

    ctmp = -one / (c * imPQ_M);
    pB_pa = ctmp * pB_prho_M2 + B_M2 * halft;
    complex<double> y5M2 = -v0 * pA_prho_M2 * ctmp + tmp2 * lnB_M2 +
                           a * tmp2 * pB_pa / B_M2 - tmp1 * imPQ_M;

    ctmp = -one / (c * _imPQ_N);
    pB_pa = ctmp * pB_prho_N1 + B_N1 * halft;
    complex<double> y5N1 = -v0 * pA_prho_N1 * ctmp + tmp2 * lnB_N1 +
                           a * tmp2 * pB_pa / B_N1 - tmp1 * _imPQ_N;

    ctmp = -one / (c * imPQ_N);
    pB_pa = ctmp * pB_prho_N2 + B_N2 * halft;
    complex<double> y5N2 = -v0 * pA_prho_N2 * ctmp + tmp2 * lnB_N2 +
                           a * tmp2 * pB_pa / B_N2 - tmp1 * imPQ_N;
    ...
    // M1
    complex<double> pd_pc =
        (tmp - one / kes_M1) * pd_prho_M1 + c * _msqr / d_M1;
    complex<double> pA1_pc = m_M1 * calp_M1 * halft * pd_pc;
    complex<double> pA2_pc =
        tmp * pA2_prho_M1 -
        one / _imPQ_M * (two / (T * kes_M1) + one) * pA1_prho_M1 +
        c * halft * A1_M1;
    complex<double> pA_pc = pA1_pc / A2_M1 - A_M1 / A2_M1 * pA2_pc;
    complex<double> y6M1 = -v0 * pA_pc - tmp1 * lnB_M1 +
    ...
    pA2_pc = tmp * pA2_prho_M2 -
             one / imPQ_M * (two / (T * kes_M2) + one) * pA1_prho_M2 +
             c * halft * A1_M2;
    pA_pc = pA1_pc / A2_M2 - A_M2 / A2_M2 * pA2_pc;
    complex<double> y6M2 = -v0 * pA_pc - tmp1 * lnB_M2 +
    ...
    pA2_pc = tmp * pA2_prho_N1 -
             one / (_imPQ_N) * (two / (T * kes_N1) + one) * pA1_prho_N1 +
             c * halft * A1_N1;
    pA_pc = pA1_pc / A2_N1 - A_N1 / A2_N1 * pA2_pc;
    complex<double> y6N1 = -v0 * pA_pc - tmp1 * lnB_N1 +
    ...
    pA2_pc = tmp * pA2_prho_N2 -
             one / (imPQ_N) * (two / (T * kes_N2) + one) * pA1_prho_N2 +
             c * halft * A1_N2;
    pA_pc = pA1_pc / A2_N2 - A_N2 / A2_N2 * pA2_pc;
    complex<double> y6N2 = -v0 * pA_pc - tmp1 * lnB_N2 +
...
void JacHes(double *p, double *jac, int m, int n, void *data) {
    ...
    for (l = k = 0; l < n; ++l) {
        ...
        double pa1 = 0.0, pa2 = 0.0, pb1 = 0.0, pb2 = 0.0, pc1 = 0.0, pc2 = 0.0,
               prho1 = 0.0, prho2 = 0.0, pv01 = 0.0, pv02 = 0.0;
...
int main() {
    ...
    // array of strikes
    double karr[] = {93.71, 86.03, 81.12, 77.60, 74.70, 72.16, 66.99,
    ...
    double c =
        0.25;  // sigma                           |  variance of volatility
    double rho = -0.8;  // rho                             |  correlation
    ...
    double x[40];
    fHes(pstar, x, m, n, (void *)&market);  // Lin: data is merket paras, but
                                            // why void? What does it mean?
    for (int j = 0; j < n; j++) {
        std::cout << std::fixed << std::setprecision(16);
        std::cout << "strike_price  " << karr[j] << std::endl;
        std::cout << "maturity      " << tarr[j] << std::endl;
        std::cout << "SPXcall_price " << x[j] << std::endl << std::endl;
    }
    double y[200];
    JacHes(pstar, y, m, n, (void *)&market);
    for (int j = 0; j < 200; j++) {
        std::cout << std::setprecision(16);
        std::cout << "gradient" << j << "   " << y[j] << std::endl;
    }
}  // The End
